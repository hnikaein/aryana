...
while (1) {
    ...
    fprintf(stdout, "%s\t%d\t%s\t%"PRIu64"\t%u\t%s\t%s\t%s\t%lld\t%s\t%s\n \n",qname, flag, rname, pos,mapq, cigar,rnext,pnext, tlen,seq_string,quality_string);
        ...
        if(strstr(cigar,"*"))
            notAlignedReads++;
        fprintf(stdout, "number of not aligned reads: %ull \n number of reads with wrong alignment : %ull \n total reads : %ull \n percentage :%f \n",notAlignedReads, badAlignedReads, readNum , badAlignedReads/readNum * 100);
