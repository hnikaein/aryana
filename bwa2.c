...
void PrintRefSeq(unsigned long long start, unsigned long long end, unsigned long long seq_len, int endl) {
    unsigned long long k;
    for (k = start; k <= end; k++)
        fprintf(stderr, "%c", atom2[(unsigned short)getNuc(k, seq_len)]);
    if (endl) fprintf(stderr, "\n");
}

void GetRefSeq(unsigned long long start, unsigned long long end, unsigned long long seq_len, char * seq) {
    unsigned long long k;
    for (k = start; k <= end; k++)
        seq[k-start] = atom2[(unsigned short)getNuc(k, seq_len)];
    seq[k-start] = 0;
}
...
//	char ** ttt2 = ttt;
    ...
    for (ii=best_num; ii<best_size; ii++)
        if (best[ii]!=-1)
        {
            ...
            if (debug >= 1) {
                fprintf(stderr, "index: %llu, cigar: %s, errors: %d\n", (unsigned long long) table[best[ii]].index,  cigar[ii], tmp_errors);
                PrintSeq(seq->seq, seq->len, 1);
                PrintRefSeq(table[best[ii]].index, table[best[ii]].index + seq->len - 1, bwt->seq_len, 1);
                /*
                fprintf(stderr,"%llu %d\n",(unsigned long long) table[best[ii]].index,best[ii]);
                fprintf(stderr, "seq->index: %llu\n", (unsigned long long) seq->index);
