...
void multiAligner(global_vars * g) {
    ...
    while(true) {
        ...
        for(j=0; j<n_seqs; j++) {
            ...
            struct report* rpt = align_read(g, buffer, cigar, cigar2, &seqs[j], (g->args->paired)? &seqs2[j] : 0, table, table2,total_seqs + j + 1, d,arr, tmp_cigar);
            report_alignment_results(rpt->g, rpt->buffer, rpt->cigar,rpt->cigar2, rpt->seq,rpt->seq2, rpt->table, rpt->table2, rpt->can, rpt->can2, rpt->canNum, rpt->canNum2, rpt->penalty, rpt->penalty2);
