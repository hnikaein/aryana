...
mem_alnreg_v mem_align1(const mem_opt_t *opt, const bwt_t *bwt, const bntseq_t *bns, const uint8_t *pac, int l_seq,
                        const char *seq_) {   // the difference from mem_align1_core() is that this routine: 1) calls mem_mark_primary_se(); 2) does not modify the input sequence
    extern mem_alnreg_v
    mem_align1_core(const mem_opt_t *opt, const bwt_t *bwt, const bntseq_t *bns, const uint8_t *pac, int l_seq,
                    char *seq, void *buf);
...
void mem_reg2ovlp(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, bseq1_t *s, mem_alnreg_v *a) {
    ...
    for (i = 0; i < a->n; ++i) {
        ...
        kputw(qe, &str);
        kputc('\t', &str);
        kputs(bns->anns[rid].name, &str);
        kputc('\t', &str);
        kputw(bns->anns[rid].len, &str);
        kputc('\t', &str);
        kputw(pos, &str);
        kputc('\t', &str);
        kputw(pos + (re - rb), &str);
...
char **mem_gen_alt(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, const mem_alnreg_v *a, int l_query,
