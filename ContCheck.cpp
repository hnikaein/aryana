...
void
PrintOutput(FILE *f, string name = "NA", string chr1 = "NA", long long pos1 = 0, int penalty1 = 0, string chr2 = "NA",
            long long pos2 = 0, int penalty2 = 0, char *samSeq = NULL, char *cigar = NULL, char *seq1 = NULL,
            char *seq2 = NULL) {
    if (!realPos) {
        fprintf(f, "%s\t%s\t%lld\t%d", name.c_str(), chr1.c_str(), pos1, penalty1);
        if (seqOutput) fprintf(f, "\t%s\t%s\t%s", samSeq, cigar, seq1);
    } else {
        fprintf(f, "%s\t%s\t%lld\t%d\t%s\t%lld\t%d", name.c_str(), chr1.c_str(), pos1, penalty1, chr2.c_str(), pos2,
                penalty2);
...
// Parsing Arugments
    ...
    if (genomeFile == "") {
        cerr
                << "Arguments: -g <reference genome, mandatory> -i <alignment sam file> -o <result file> -S (print SAM, CIGAR and alignment sequences in output)"
                << endl \

                << "-b (bisulfite-seq input, default: DNA-seq)  -s (use only if reads are generated via BisSimul or contain the true positions in a similar way)"
                << endl;
        exit(1);
    }
