...
//                 		index = 0;
            ...
            add(bwt, rindex / tag_size, score, level, rindex, best, best_size, best_found, table, i - limit + 1, limit,
                index, len, groupid_last); // if level changed, check the find_value in hash.c
            if (args->debug > 2)
                fprintf(stderr,
