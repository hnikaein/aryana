    ...
    if (argc != 2) {
        ...
        while (1) {
            ...
            sscanf(line, "%s\t%d\t%s\t%"PRIu64"\t%u\t%s\t%s\t%s\t%lld\t%s\t%s\n", qname, &flag, rname, &pos, &mapq,
                   cigar, rnext, pnext, &tlen, seq_string, quality_string);
...
void computeMethylation() {
    ...
    if (cytosines.size() != 0) {
        ...
        if (lines[0].pos > lastchecked || strcmp(cytosines[cytosines.size() - 1].chr,
                                                 lines[0].chr)) {//if chrom has changed or start of the read has passed lastchecked pos
