...
bwa_seq_t *bwa_read_seq2(bwa_seqio_t *bs, int n_needed, int *n, int mode, int trim_qual, bwa_seq_t *seqs)
{
	bwa_seq_t *p;
	kseq_t *seq = bs->ks;
...
bwa_seq_t *bwa_read_seq(bwa_seqio_t *bs, int n_needed, int *n, int mode, int trim_qual)
{
	...
	if (l_bc > BWA_MAX_BCLEN) { //Afsoon: What is barcode?
		fprintf(stderr, "[%s] the maximum barcode length is %d.\n", __func__, BWA_MAX_BCLEN);
		return 0;
	}
	fprintf(stderr,"AAAA\n");
...
void bwa_free_read_seq2(int n_seqs, bwa_seq_t *seqs)
{
	int i, j;
	for (i = 0; i != n_seqs; ++i) {
		bwa_seq_t *p = seqs + i;
