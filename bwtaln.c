...
//-----------------------------------------------------------------------Milad
void countrepeat(const char *prefix){
	bwt_t *bwt;
	{ // load BWT
	...
	//fprintf(stderr, "begining...\n");
	const int maxoccur = 100000;
	int occur[maxoccur];
	memset(occur, 0, sizeof(occur));
	FILE *fin=fopen(prefix,"r");
	const int BSIZE=10000;
	char buf[BSIZE];
	ubyte_t str[BSIZE];

	int i=0, K = 25;
	bwtint_t down, up, total;
	int bufstart = 0, bufend = 0;
	//fprintf(stderr, "before reading\n");
	while (! feof(fin)) {
		if (bufend + 1000 > BSIZE) {
			memcpy(buf, buf + bufstart, bufend - bufstart);
			bufend -= bufstart;
			bufstart = 0;
			//fprintf(stderr, "shifting buffer... bufstart = %d, bufend = %d\n", bufstart, bufend);
		}
		i = fscanf(fin, "%s", buf + bufend);
		if (i == EOF) break;
		i = strlen(buf + bufend);
		//fprintf(stderr, "read %s\n", buf + bufend);
		//fprintf(stderr, "reading one line with size = %d \n", i);
		int j;
		for (j = 0; j < i; j++)
			switch(buf[j + bufend]) {
			case 'A': buf[j + bufend] = 0; break;
			case 'C': buf[j + bufend] = 1; break;
			case 'G': buf[j + bufend] = 2; break;
			case 'T': buf[j + bufend] = 3; break;
			default: fprintf(stderr, "reading something wrong : %c\n", buf[j + bufend]);
			}
		bufend += i;
		//fprintf(stderr, "before exact match \n");
		//fprintf(stderr, "proceccing... bufstart = %d, bufend = %d, total = \n", bufstart, bufend);//, total);
		while (bufend - bufstart >= K){
			up = down = 0;
			//for(i=0,j=bufend - 1; i<K; i++, j--)
			for(i=0,j=bufstart;i<K;i++, j++)
				str[i] = buf[j];
			if(!bwt_match_exact(bwt, K, (const ubyte_t *) str, &down, &up)){
				showerr(K, (const ubyte_t *) str);
				fprintf(stderr, "Error: not matched.\n");
			} else {
				total = up + 1 - down;
				//fprintf(stderr, "total = %d, up = %d, down = %d\n", total, up, down);
				if(total < maxoccur)
					occur[total]++;
				else
					occur[maxoccur - 1]++;
			...
			bufstart++;
			//if(bufstart == 100)
				//fprintf(stderr, "proceccing... bufstart = %d, bufend = %d, total = %d\n", bufstart, bufend, total);
		}
	}

	int max = 0;
	for(i=1;i<maxoccur;i++){
		if((i < maxoccur - 1) && (occur[i] % i != 0))
			fprintf(stderr, "ERROR!!!! i = %d, total = %d\n", i, occur[i]);
		occur[i] /= i;
	}
...
int bwa_aln2(int argc, char *argv[])
{
	int c, opte = -1;
	gap_opt_t *opt;
	...
	options.min_dis=0;
	options.max_dis=0;
	while ((c = getopt(argc, argv, "n:o:e:i:d:l:k:cLR:m:t:NM:O:E:q:f:b012IYB:Pg:G:")) >= 0) {
		switch (c) {
		case 'n':
		...
		case 'P': options.paired=1; break;
		case 'g': options.min_dis=atoi(optarg); break;
		case 'G': options.max_dis=atoi(optarg); break;
		default: return 1;
		}
	}
	if (opte > 0) {
	...
	if (options.paired)
	{
		if (options.min_dis==0)
			options.min_dis=500;
		if (options.max_dis==0)
			options.min_dis=options.min_dis+100;
	}
