...
//		cerr << "ChromPos: " << chromPos[i] << " ChromLen: " << chromLen[i] << endl;
    ...
    if (!gs || !chromNum) {
        cerr
                << "Error: Either size or number of chromosomes of the reference genome is zero. \nIt should be FASTA format with chromosome names provided.\n";
        exit(1);
    }
}
...
void AssignMethylationRatio(string cpgIslandFile) {
    unsigned short cpg1 = (unsigned short) (cpg * 100), noncpg1 = (unsigned short) (noncpg *
                                                                                    100), island1 = (unsigned short) (
            island * 100);
    char chr[10000];
    long long position;
    double m;
    ...
    for (unsigned int j = 0; j < islands.size(); j++) {
        for (long long i = islands[j].wStart + chromPos[islands[j].chr];
             i <= islands[j].wEnd + chromPos[islands[j].chr]; i++) {
...
// Simulating CpG-island reads
    ...
    for (int i = 0; i < ni; i++) {
        ...
        do {
            if (tries++ > maxTries) {
                cerr << "Error: Could not find any suitable location for CpG-island reads after " << maxTries
                     << " tries." << endl;
                exit(-1);
            }
            is = lrand() % islands.size();
            p = rand() % (islands[is].wEnd - (islands[is].wStart - readl + 1) + 1) + islands[is].wStart - readl + 1 +
                chromPos[islands[is].chr]; // To have at least 1bp overlap between read and CpG-island
...
void WriteMethylationRatios() {
    if (methOutFile != "") {
        cerr << "Writing methylation ratio of the nucleotides in: " << methOutFile << endl
             << "Please be patient, this might take some time depending on the genome size" << endl;
...
void WriteReadCounts() {
    if (countOutFile != "") {
        cerr << "Writing the number of reads covering each cytosine (converted, unconverted, ratio) in: "
             << countOutFile << endl << "Please be patient, this might take some time depending on the genome size"
             << endl;
        ...
        for (int i = 0; i < chromNum; i++)
            for (long long j = chromPos[i]; j < chromPos[i + 1]; j++)
                if (totalCount[j] > 0)
                    fprintf(f, "%s\t%lld\t%d\t%d\t%.2f\n", chromName[i].c_str(), j + 1 - chromPos[i], totalCount[j],
                            methylCount[j], (double) methylCount[j] / totalCount[j]);
        fclose(f);
    }
}
...
void Usage() {
    cerr << "Arguments:" << endl <<
         ...
         "     -o/--output    file             output file name without extension, in FASTQ format, default=stdout"
         << endl <<
         ...
         "     -s/--snp       int              number of SNPs to be introduced to the reference genome, default=0"
         << endl <<
         ...
         "     --ff/--fr/--rf                  the relative orientation of the paired reads, default=ff (forward/forward). fr and rf turn --neg on."
         << endl << endl <<
         ...
         "     --ch           float            methylation ratio of CH dinucleotides, between 0 and 1, default=0.01"
         << endl <<
...
// Parsing Arugments
    ...
    if (pairMinDis > pairMaxDis || pairMinDis < 0) {
        cerr
                << "Error: Min distance between pair of reads should be a non-negative integer, smaller than max distance between them"
                << endl;
        exit(1);
    }
