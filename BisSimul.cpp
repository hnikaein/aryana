...
void AssignMethylationRatio(char * cpgIslandFile) {
	...
	{
		if (genome[i] == 'c' || genome[i] == 'C') 
			if (i + 1 < gs && (genome[i+1] == 'g' || genome[i+1] == 'G')) meth[i] = cpg1; else meth[i] = noncpg1;
		if (genome[i] == 'g' || genome[i] == 'G') 
			if (i > 0 && (genome[i-1] == 'c' || genome[i-1] == 'C')) meth[i] = cpg1; else meth[i] = noncpg1;
	}
								
	cerr << "Processing CpG island locations from file: " <<  annotationFile << endl;
    ifstream f(cpgIslandFile);
    if (! f.is_open()) {
...
void revcomp(char * a, long long l) {
	...
	for (long long i = 0; i < l; i++)
		switch (b[i]) {
			case 'a': case 'A': a[l - i - 1] = 'T'; break;
			case 'c': case 'C': a[l - i - 1] = 'G'; break;
			case 'g': case 'G': a[l - i - 1] = 'C'; break;
			case 't': case 'T': a[l - i - 1] = 'A'; break;
			default: a[l-i-1] = 'N';
		};
}
...
void SimulateReads(char * outputFile) {
	FILE * f = fopen(outputFile, "w");
...
// Parsing Arugments
	if (argc < 6) { 
        	cerr << "Usage is -g <reference genome> -n <number of simulated reads from whole genome>  -ni <number of simulated reads from CpG-Islands>" << endl <<
					"-l <length of each read> -m (mask repeats) -ch <methylation ratio of CH dinucleotides> -cg <methylation ratio of CG dinucleotides>" << endl << 
					...
					"-o <output file> -neg (generate reads from negative strand) -p (generate pcr amplified reads)" << endl;
        	exit(1);
    }  
