...
int bwa_pemerge(const pem_opt_t *opt, bseq1_t x[2]) {
    ...
    if (r.score < opt->T) {
        ret = -1;    // poor alignment
        goto pem_ret;
    }
    if (r.tb < r.qb) {
        ret = -2;    // no enough space for the left end
        goto pem_ret;
    }
    if (x[0].l_seq - r.te > x[1].l_seq - r.qe) {
        ret = -3;    // no enough space for the right end
        goto pem_ret;
    }
    if ((double) r.score2 / r.score >= MAX_SCORE_RATIO) {
        ret = -4;    // the second best score is too large
        goto pem_ret;
    }
    if (r.qe - r.qb != r.te - r.tb) {
        ret = -5;    // we do not allow gaps
        goto pem_ret;
    }
