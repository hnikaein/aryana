        ...
        switch (tolower(cigar[cigarpos++])) {
            case 'm':
                for (p = pos; p < pos + num; p++) {
                    if (p >= chromSize[ch]) {
                        cerr << "Error: read " << qname << " mapping position is higher than the length of chromosome "
                             << chr << endl;
                        return;
                    } else {
...
// Parsing Arugments
    ...
    if (chromSizesFile == "") {
        cerr
                << "Arguments: -c <chromosome sizes, mandatory> -i <alignment sam file> -w <result wig file> -s (strand-specific output, the WIG file will have both positive and negative values)"
                << endl <<
                "           -n (normalize, default=no) -f <scaling factor, default=1e+8, sets -n on> -W <negative strand wig file, sets -s on, without it both strands are written to the -o specified file>"
                << endl <<
                "           -e (end of each interval will be less than start of the next interval. default: end of each interval can be equal to start of the next interval)"
                << endl;
        exit(1);
    }
